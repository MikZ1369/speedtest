...
abstract class ServerSelector(servers: Array<TestPoint>, timeout: Int) {
    ...
    fun addTestPoint(t: JSONObject?) {
        check(state == NOT_STARTED) { "Cannot add test points at this time" }
        servers.add(TestPoint(t!!))
    }

    fun addTestPoints(a: JSONArray) {
        check(state == NOT_STARTED) { "Cannot add test points at this time" }
    ...
    private operator fun next() {
        ...
        synchronized(mutex) {
            ...
            val ps: PingStream = object : PingStream(tp!!.server!!, tp.pingURL!!, PINGS, SpeedtestConfig.ONERROR_FAIL, timeout, timeout, -1, -1, null) {
                ...
                override fun onPong(ns: Long): Boolean {
                    val p = ns / 1000000f
                    if (tp!!.ping == (-1).toFloat() || p < tp.ping) tp.ping = p
