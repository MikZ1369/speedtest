package com.mik.speedtest.core.worker

import android.util.Log
import com.mik.speedtest.core.base.Connection
import com.mik.speedtest.core.base.Utils
import com.mik.speedtest.core.config.SpeedtestConfig
import com.mik.speedtest.core.config.TelemetryConfig
import com.mik.speedtest.core.download.DownloadStream
import com.mik.speedtest.core.getIP.GetIP
import com.mik.speedtest.core.log.Logger
import com.mik.speedtest.core.ping.PingStream
import com.mik.speedtest.core.serverSelector.TestPoint
import com.mik.speedtest.core.telemetry.Telemetry
import com.mik.speedtest.core.upload.UploadStream
import org.json.JSONObject
import java.lang.Double.MAX_VALUE
import java.util.*

abstract class SpeedtestWorker(private val backend: TestPoint, config: SpeedtestConfig?, telemetryConfig: TelemetryConfig?) : Thread() {
        ...
        private get() {
            ...
            val g: GetIP = object : GetIP(c, backend.getIpURL!!, config.getIP_isp, config.getGetIP_distance()) {
                override fun onDataReceived(data: String?) {
                    ...
                    ipIsp = data!!
                    try {
                        data = JSONObject(data).getString("processedString")
                    } catch (t: Throwable) {
                    }
    ...
    private fun dlTest() {
        ...
        for (i in streams.indices) {
            streams[i] = object : DownloadStream(backend.server!!, backend.dlURL!!, config.getDl_ckSize(), config.getErrorHandlingMode(), config.dl_connectTimeout, config.dl_soTimeout, config.dl_recvBuffer, config.dl_sendBuffer, log) {
                override fun onError(err: String?) {
        ...
        while (true) {
            ...
            if (graceTimeDone) {
                ...
                if (config.time_auto) {
                    val b = 3.2 * speed / 100000.0
                    bonusT += if (b > 400) 400 else b.toLong()
                }
                val progress = (t + bonusT) / (config.getTime_dl_max() * 1000).toDouble()
                speed = speed * 8 * config.getOverheadCompensationFactor() / if (config.useMebibits) 1048576.0 else 1000000.0
                dl = speed
        ...
        if (stopASAP) return
        Log.d("Test", "Download: " + dl + " (took " + (System.currentTimeMillis() - start) + "ms)")
    ...
    private fun ulTest() {
        ...
        for (i in streams.indices) {
            streams[i] = object : UploadStream(backend.server!!, backend.ulURL!!, config.getUl_ckSize(), config.getErrorHandlingMode(), config.ul_connectTimeout, config.ul_soTimeout, config.ul_recvBuffer, config.ul_sendBuffer, log) {
                override fun onError(err: String?) {
        ...
        while (true) {
            ...
            if (graceTimeDone) {
                ...
                var speed: Double = totUploaded / ((if (t < 100) 100.0.toDouble() else t.toDouble()) / 1000.0.toDouble())
                if (config.time_auto) {
                    val b = 3.2 * speed / 100000.0
                    bonusT += if (b > 400) 400 else b.toLong()
                }
        ...
        if (stopASAP) return
        Log.d("Test","Upload: " + ul + " (took " + (System.currentTimeMillis() - start) + "ms)")
    ...
    private fun pingTest() {
        ...
        val ps: PingStream = object : PingStream(backend.server!!, backend.pingURL!!, config.getCount_ping(), config.getErrorHandlingMode(), config.ping_connectTimeout, config.ping_soTimeout, config.ping_recvBuffer, config.ping_sendBuffer, log) {
            ...
            override fun onPong(ns: Long): Boolean {
                ...
                onPingJitterUpdate(ping, jitter, if (progress > 1) 1.0 else progress)
                return !stopASAP
            }
    ...
    private fun sendTelemetry() {
        ...
        try {
            ...
            val t: Telemetry = object : Telemetry(c, telemetryConfig.path!!, telemetryConfig.telemetryLevel, ipIsp, config.telemetry_extra, if (dl == -1.0) "" else java.lang.String.format(Locale.ENGLISH, "%.2f", dl), if (ul == -1.0) "" else java.lang.String.format(Locale.ENGLISH, "%.2f", ul), if (ping == -1.0) "" else java.lang.String.format(Locale.ENGLISH, "%.2f", ping), if (jitter == -1.0) "" else java.lang.String.format(Locale.ENGLISH, "%.2f", jitter), log.log) {
                override fun onDataReceived(data: String?) {
