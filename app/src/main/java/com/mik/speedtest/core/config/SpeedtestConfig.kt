...
class SpeedtestConfig {
    ...
    constructor(dl_ckSize: Int, ul_ckSize: Int, dl_parallelStreams: Int, ul_parallelStreams: Int, dl_streamDelay: Int, ul_streamDelay: Int, dl_graceTime: Double, ul_graceTime: Double, dl_connectTimeout: Int, dl_soTimeout: Int, ul_connectTimeout: Int, ul_soTimeout: Int, ping_connectTimeout: Int, ping_soTimeout: Int, dl_recvBuffer: Int, dl_sendBuffer: Int, ul_recvBuffer: Int, ul_sendBuffer: Int, ping_recvBuffer: Int, ping_sendBuffer: Int, errorHandlingMode: String, time_dl_max: Int, time_ul_max: Int, time_auto: Boolean, count_ping: Int, telemetry_extra: String, overheadCompensationFactor: Double, getIP_isp: Boolean, getIP_distance: String, useMebibits: Boolean, test_order: String) {
    ...
    constructor(json: JSONObject) {
        try {
            if (json.has("dl_ckSize")) dl_ckSize = json.getInt("dl_ckSize")
            if (json.has("ul_ckSize")) ul_ckSize = json.getInt("ul_ckSize")
            if (json.has("dl_parallelStreams")) dl_parallelStreams = json.getInt("dl_parallelStreams")
            if (json.has("ul_parallelStreams")) ul_parallelStreams = json.getInt("ul_parallelStreams")
            if (json.has("dl_streamDelay")) dl_streamDelay = json.getInt("dl_streamDelay")
            if (json.has("ul_streamDelay")) ul_streamDelay = json.getInt("ul_streamDelay")
            if (json.has("dl_graceTime")) dl_graceTime = json.getDouble("dl_graceTime")
            if (json.has("ul_graceTime")) ul_graceTime = json.getDouble("ul_graceTime")
            if (json.has("dl_connectTimeout")) dl_connectTimeout = json.getInt("dl_connectTimeout")
            if (json.has("ul_connectTimeout")) ul_connectTimeout = json.getInt("ul_connectTimeout")
            if (json.has("ping_connectTimeout")) ping_connectTimeout = json.getInt("ping_connectTimeout")
            if (json.has("dl_soTimeout")) dl_soTimeout = json.getInt("dl_soTimeout")
            if (json.has("ul_soTimeout")) ul_soTimeout = json.getInt("ul_soTimeout")
            if (json.has("ping_soTimeout")) ping_soTimeout = json.getInt("ping_soTimeout")
            if (json.has("dl_recvBuffer")) dl_recvBuffer = json.getInt("dl_recvBuffer")
            if (json.has("ul_recvBuffer")) ul_recvBuffer = json.getInt("ul_recvBuffer")
            if (json.has("ping_recvBuffer")) ping_recvBuffer = json.getInt("ping_recvBuffer")
            if (json.has("dl_sendBuffer")) dl_sendBuffer = json.getInt("dl_sendBuffer")
            if (json.has("ul_sendBuffer")) ul_sendBuffer = json.getInt("ul_sendBuffer")
            if (json.has("ping_sendBuffer")) ping_sendBuffer = json.getInt("ping_sendBuffer")
            if (json.has("errorHandlingMode")) errorHandlingMode = json.getString("errorHandlingMode")
            if (json.has("time_dl_max")) time_dl_max = json.getInt("time_dl_max")
            if (json.has("time_ul_max")) time_ul_max = json.getInt("time_ul_max")
            if (json.has("count_ping")) count_ping = json.getInt("count_ping")
            if (json.has("telemetry_extra")) telemetry_extra = json.getString("telemetry_extra")
            if (json.has("overheadCompensationFactor")) overheadCompensationFactor = json.getDouble("overheadCompensationFactor")
            if (json.has("getIP_isp")) getIP_isp = json.getBoolean("getIP_isp")
            if (json.has("getIP_distance")) getIP_distance = json.getString("getIP_distance")
            if (json.has("test_order")) test_order = json.getString("test_order")
            if (json.has("useMebibits")) useMebibits = json.getBoolean("useMebibits")
            check()
    ...
    fun clone(): SpeedtestConfig {
        return SpeedtestConfig(dl_ckSize, ul_ckSize, dl_parallelStreams, ul_parallelStreams, dl_streamDelay, ul_streamDelay, dl_graceTime, ul_graceTime, dl_connectTimeout, dl_soTimeout, ul_connectTimeout, ul_soTimeout, ping_connectTimeout, ping_soTimeout, dl_recvBuffer, dl_sendBuffer, ul_recvBuffer, ul_sendBuffer, ping_recvBuffer, ping_sendBuffer, errorHandlingMode, time_dl_max, time_ul_max, time_auto, count_ping, telemetry_extra, overheadCompensationFactor, getIP_isp, getIP_distance, useMebibits, test_order)
    }
